...
-- 7 Get all users with account balances that are more than $100.
    SELECT username FROM accounts WHERE account_balance>100.00;
-- 8 Add a new record.
...
-- 9 The bank is losing money in Miami and Phoenix and needs to unload low transaction customers: Delete users that reside in miami OR phoenix and have completed fewer than 5 transactions.
    SELECT username FROM accounts WHERE city='miami' AND transactions_completed<5 or city='phoenix' AND transactions_completed<5;